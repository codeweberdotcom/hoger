	...
	?>
		...
		<?php if ($order->has_status('failed')) : ?>
			...
			<div class="col-12">
				...
				<p class="woocommerce-notice woocommerce-notice--error woocommerce-thankyou-order-failed-actions woocommerce-error alert alert-danger alert-icon mt-3 list-unstyled">
					<i class="uil uil-times-circle"></i> <a href="<?php echo esc_url($order->get_checkout_payment_url()); ?>" class="button pay"><?php esc_html_e('Pay', 'woocommerce'); ?></a>
		...
		<?php else : ?>
			...
			<div class="col-12">
				<div class="woocommerce-notice-info alert alert-blue alert-icon mb-6 mt-3 col-12">
			...
			<div class="col-12 mb-4">
				<ul class="woocommerce-order-overview woocommerce-thankyou-order-details order_details mb-0 list-group list-group-horizontal-lg">
					<li class="woocommerce-order-overview__order order list-group-item flex-grow-1">
						<?php esc_html_e('Order number:', 'woocommerce'); ?>
						<strong><?php echo $order->get_order_number(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped 
									?></strong>
					</li>
					<li class="woocommerce-order-overview__date date list-group-item flex-grow-1">
					...
					<?php if (is_user_logged_in() && $order->get_user_id() === get_current_user_id() && $order->get_billing_email()) : ?>
						<li class="woocommerce-order-overview__email email list-group-item flex-grow-1">
							<?php esc_html_e('Email:', 'woocommerce'); ?>
							<strong><?php echo $order->get_billing_email(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped 
										?></strong>
						</li>
					<?php endif; ?>
					<li class="woocommerce-order-overview__total total list-group-item flex-grow-1">
					...
					<?php if ($order->get_payment_method_title()) : ?>
						<li class="woocommerce-order-overview__payment-method method list-group-item flex-grow-1">
